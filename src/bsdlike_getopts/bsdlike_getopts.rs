...
// XXX Note: This is a simple modification to libgetopts that ignores all
// options after a free argument.  Although this could potentially allow for a
// more efficient signature, it's unchanged for compatibility.  The only change
// from libgetopts is the six lines after 'free.push(cur);', plus the crate
// name, but thanks to privacy, we can't just modify one function and import
// the rest...
    ...
    where F: FnMut(&str) -> bool
{
    ...
    let mut machine = |state: &mut SplitWithinState, cont: &mut bool, (i, c): (usize, char)| -> bool {
    ...
    ss.char_indices().all(|x| machine(&mut state, &mut cont, x));
    ...
    while cont &&
        ...
        machine(&mut state, &mut cont, (fake_i, ' '));
        fake_i += 1;
    }
    cont
}
